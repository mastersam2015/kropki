			...
			//alert("rx " + rx + " ==  pomx "+pomx+", ry " + ry +" == pomy "+pomy);
				...
				if((jeden!=1) && (((rx+20==pomx) || (rx-20==pomx) || (rx==pomx)) && ((ry+20==pomy) || (ry-20==pomy) || (ry==pomy)))){
				  ...
				  for (var f=0;f<=400;f++){
					  ...
					  if(tabx[f]==rx && ry==taby[f] && tabc[f] == "n"){
...
//ctx.drawImage(img,rx-10,ry-10); 
			  window.location.href="index2.php?kolor=n&x="+rx+"&y="+ry;
	...
	//alert(data);
		  	if(data==2){
				...
				window.location.href="index2.php?reset=4";
					...
					if(gox!=0 || ostring2 !=0){
						// ostring != 0
						window.location.href="index2.php?reset=1&kolor=n&x="+gox+"&y="+goy+"&ostring="+ostring;
					}
					//alert(gox);
				}
...
fclose($plik);
		...
		echo "<script>window.location.href='index2.php?reset=1'</script>";
...
fclose($plik);
	...
	$poms=explode("a",$tab2[$i]);
		...
		if(!empty($tab2[$i+1]) and $zoms[2]=="c"){
...
echo "<br>".$tekst4.":".$tekst5;
	...
	<br>
	<br>
	<input type="button" value="reflesh" onclick="window.location.href='index2.php?reset=1'">
